...
from .utils import create_data_model_home, timeparser_model_home
...
def fasttext_downloader(model_name):
    ...
    if model_name not in models:
        message = 'timeparser-download: Couldn\'t find a model called "{}". Supported models are: {}'.format(
            model_name, ", ".join(models.keys())
        )
        raise FastTextModelNotFoundException(message)

    models_directory_path = os.path.join(timeparser_model_home, (model_name + ".bin"))

    if not Path(models_directory_path).is_file():
        model_url = models[model_name]
        logging.info(
            'timeparser-download: Downloading model "{}" from "{}"...'.format(
        ...
        try:
            ...
            raise Exception(
                "timeparser-download: Fasttext model cannot be downloaded due to HTTP error"
            ) from e
    else:
        logging.info(
            'timeparser-download: The model "{}" is already downloaded'.format(
                model_name
            )
        )
