...
from timeparser_cli.exceptions import FastTextModelNotFoundException
from timeparser_cli.fasttext_manager import fasttext_downloader
from timeparser_cli.utils import create_data_model_home, timeparser_model_home
...
def _load_fasttext_model():
    ...
    downloaded_models = [
        file for file in os.listdir(timeparser_model_home) if file in _supported_models
    ]
    if not downloaded_models:
        fasttext_downloader(_DEFAULT_MODEL)
        return _load_fasttext_model()
    model_path = os.path.join(timeparser_model_home, downloaded_models[0])
