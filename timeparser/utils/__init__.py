...
from timeparser.timezone_parser import StaticTzInfo, _tz_offsets
...
def apply_timeparser_timezone(utc_datetime, offset_or_timezone_abb):
    for name, info in _tz_offsets:
...
def apply_timezone(date_time, tz_string):
    ...
    new_datetime = apply_timeparser_timezone(date_time, tz_string)
...
def get_logger():
    setup_logging()
    return logging.getLogger("timeparser")
